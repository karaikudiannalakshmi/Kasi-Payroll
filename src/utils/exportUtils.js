...
export function exportBankUpload(rows, yearMonth, debitAccountOverride) {
  ...
  const setCell=(addr,value,isText,isCenter,isWrap)=>{
    ws[addr]={v:value,t:isText?'s':(typeof value==='number'?'n':'s'),z:isText?'@':(typeof value==='number'?'0':'@'),
      s:{border,alignment:{horizontal:isCenter?'center':'left',vertical:'center',wrapText:isWrap||false},font:{name:'Calibri',sz:11}}};
  };
  setCell('A1',H_A,true,false,true); setCell('B1',H_B,true,false,true);
  setCell('C1',H_C,true,false,true); setCell('D1',H_D,true,true,true); setCell('E1',H_E,true,false,true);
  rows.forEach((r,i)=>{
    const row=i+2;
...
export function exportMonthlyReport(salaryRows, advances, loans, yearMonth) {
  ...
  const wb = XLSX.utils.book_new();
  const salData = salaryRows.map((r,i)=>({'S.No':i+1,'Name':r.name,'Monthly CTC':r.monthlySalary,
    'Effective Days':+r.effectiveDays.toFixed(2),'Total Hours':+r.totalEffectiveHours.toFixed(1),
    'Gross Salary':r.grossSalary,'Advance Deduction':r.advanceDeduction||0,'Loan EMI':r.loanDeduction||0,
    'Net Pay':r.netPay,'Customer ID':r.customerId||r.beneId||''}));
  salData.push({'S.No':'','Name':'TOTAL','Monthly CTC':'','Effective Days':'','Total Hours':'',
    ...
    'Net Pay':salaryRows.reduce((s,r)=>s+r.netPay,0),'Customer ID':''});
  ...
  const advData=advances.map((a,i)=>({'S.No':i+1,'Employee':a.empName||a.empId,'Amount':a.amount,'Date':a.date||'','Status':a.deducted?'Deducted':'Pending'}));
  advData.push({'S.No':'','Employee':'TOTAL','Amount':advances.reduce((s,a)=>s+Number(a.amount||0),0),'Date':'','Status':''});
  const ws2=XLSX.utils.json_to_sheet(advData);
  XLSX.utils.book_append_sheet(wb,ws2,`Advances ${label}`);
  const loanData=loans.map((l,i)=>({'S.No':i+1,'Employee':l.empName||l.empId,'Principal':l.principalAmount,'EMI':l.emi,
    'Opening Bal':l.openingBalance,'EMI Deducted':l.emiDeducted,'Closing Bal':l.closingBalance,'Status':l.status}));
...
export function exportSalaryStatement(rows, yearMonth) {
  const label=monthLabel(yearMonth);
  const data=rows.map((r,i)=>({'S.No':i+1,'Name':r.name,'Monthly CTC':r.monthlySalary,
    'Effective Days':+r.effectiveDays.toFixed(2),'Total Hours':+r.totalEffectiveHours.toFixed(1),
    'Gross Salary':r.grossSalary,'Advance Deduction':r.advanceDeduction||0,'Loan EMI':r.loanDeduction||0,
    'Net Pay':r.netPay,'Customer ID':r.customerId||r.beneId||''}));
  data.push({'S.No':'','Name':'TOTAL','Monthly CTC':'','Effective Days':'','Total Hours':'',
    ...
    'Net Pay':rows.reduce((s,r)=>s+r.netPay,0),'Customer ID':''});
...
export function exportPayslips(rows, yearMonth) {
  const label=monthLabel(yearMonth);
  const data=rows.map((r,i)=>({'S.No':i+1,'Employee':r.name,'Month':label,
    'Monthly CTC':r.monthlySalary,'Daily Rate':+r.daily.toFixed(2),
    'Effective Days':+r.effectiveDays.toFixed(2),'Total Hours':+r.totalEffectiveHours.toFixed(1),
    'Gross Salary':r.grossSalary,'Advance Deduction':r.advanceDeduction||0,'Loan EMI':r.loanDeduction||0,'Net Salary':r.netPay}));
  const ws=XLSX.utils.json_to_sheet(data);
