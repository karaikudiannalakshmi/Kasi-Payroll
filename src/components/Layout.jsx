    ...
    <div className="flex h-screen overflow-hidden">
      <aside className="w-52 bg-gradient-to-b from-orange-800 to-red-900 text-white flex flex-col shrink-0">
        ...
        <nav className="flex-1 py-4 px-2 space-y-1 overflow-y-auto">
          {links.map(l => (
            <NavLink key={l.to} to={l.to} className={({isActive})=>`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${isActive?'bg-orange-600 text-white':'text-orange-100 hover:bg-orange-700/60'}`}>
              <span className="text-base">{l.icon}</span>{l.label}
            </NavLink>
          ))}
        </nav>
        <div className="px-4 py-3 border-t border-orange-700 space-y-1">
          <div className="text-xs text-orange-300">{user?.username}</div>
          <div className="text-xs text-orange-400 capitalize">{role}</div>
          <button onClick={handleLogout} className="w-full text-xs text-orange-200 hover:text-white py-1 text-left">🚪 Sign out</button>
        </div>
      </aside>
      <main className="flex-1 overflow-y-auto bg-amber-50"><Outlet /></main>
    </div>
  );
}
