...
export default function Salary() {
  ...
  const calculate = async () => {
    ...
    try {
      ...
      // ── Advances ──────────────────────────────────────────────────────────
      // Include ALL advances for this deductMonth regardless of deducted flag
      // (deducted flag only prevents showing in future months)
      const allAdvancesThisMonth = await getAdvances({ deductMonth: yearMonth, includeDeducted: true });
      const advMap = {};
      const advIdsByEmp = {};
      allAdvancesThisMonth.forEach(a => {
      ...
      // ── Loans ─────────────────────────────────────────────────────────────
      // Check which loans already have payment for this month
      ...
      loans.filter(l => l.status === 'active' && l.startDate <= yearMonth).forEach(l => {
        ...
        // Always show EMI — whether paid this month or not
      ...
      const rows = active.map(emp => {
        ...
        // Always use live values — savedRec only used for isSaved status
