...
import {
  ...
  getAdvances, getAllAdvancesForMonth, updateAdvance, getLoans, getLoanPayments, recordLoanPayment, saveSalaryRecord, getSalaryRecords, getNewMonthAttendance,
} from '../hooks/useFirebase';
import { calcEmployeeSalary, calcNetPay, currentYM, monthLabel, fmt, isNewSystem } from '../utils/calculations';
...
export default function Salary() {
  ...
  const calculate = async () => {
    ...
    try {
      ...
      const [employees, rawAttendance, holidays, advances, loans, savedRecords] = await Promise.all([
        getEmployees(),
      ...
      // normalise attendance: old=hours map, new={ticks,otHours,permHours}
      ...
      const rows = active.map(emp => {
        const attData = newSys
          ? (attendance[emp.id] || { ticks:{}, otHours:0, permHours:0 })
          : (attendance[emp.id] || {});
