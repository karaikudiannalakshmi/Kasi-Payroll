...
export default function Login() {
  ...
  const handleLogin = (e) => {
    ...
    try { login(username, password); } catch(err) { setError(err.message); setLoading(false); }
  };
  return (
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-8">
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          <div>
            ...
            <input className="input" type="text" value={username} onChange={e=>setUsername(e.target.value)} placeholder="admin or operator" required autoFocus autoCapitalize="none" />
          </div>
          <div>
        ...
        <p className="text-center text-xs text-gray-400 mt-6">KVKF Varanasi Kitchen Payroll System</p>
      </div>
    </div>
  );
}
